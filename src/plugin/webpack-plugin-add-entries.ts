...
export default class WebpackPluginAddEntries {
  ...
  public apply(compiler: Compiler) {
    ...
    if (this.addToEach.length) {
      compiler.options.entry = this.injectToAllEntries(
        "",
  ...
  private injectNamedToEntriesObject(
    currentEntry: EntryType | Promise<EntryType>,
    ...
    if (currentEntry === DEFAULT_ENTRY_VALUE && !this.defaultEntry) {
      ...
      return () =>
        this.injectNamedToEntriesObject(currentEntry(), namedEntries);
    } else if (typeof currentEntry.then === "function") {
      return currentEntry.then(entry =>
        this.injectNamedToEntriesObject(entry, namedEntries)
      );
    ...
    if (typeof currentEntry === "string" || Array.isArray(currentEntry)) {
      ...
      this.addToEach.forEach(entry => {
        if (entry.filter != null) {
          ...
          if (
            (entry.filter instanceof RegExp && !entry.filter.test(name)) ||
      ...
      return () =>
        this.injectToAllEntries(entryName, (currentEntry as EntryFunc)());
    } else if (typeof currentEntry.then === "function") {
      return currentEntry.then(entry =>
        this.injectToAllEntries(entryName, entry)
      );
    } else if (currentEntry === Object(currentEntry)) {
      currentEntry = Object.assign({}, currentEntry);
      Object.keys(currentEntry).forEach(entryName => {
        currentEntry[entryName] = this.injectToAllEntries(
          entryName,
